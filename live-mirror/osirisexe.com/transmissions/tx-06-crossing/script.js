...
if (!gl) {
  canvas.style.background = "#010103";
...
void main() {
  ...
  // OSIRIS.EXE locked palette — lake-midnight #0B1226, deep-indigo #1B2A5E,
  // resurrection #C9A227, agi-gold #FFC93C, bone/cream whites, duat-black #010103.
  vec3 skyTop = sCol(
    vec3(0.106, 0.165, 0.369),
    vec3(0.160, 0.250, 0.550),
    vec3(0.315, 0.254, 0.061),
    vec3(0.010, 0.016, 0.040),
    vec3(0.030, 0.050, 0.104)
  );

  vec3 skyHori = sCol(
    vec3(1.0, 0.788, 0.235),
    vec3(0.42, 0.50, 0.78),
    vec3(0.788, 0.635, 0.153),
    vec3(0.043, 0.071, 0.149),
    vec3(0.130, 0.160, 0.250)
  );

  vec3 sunCol = sCol(
    vec3(1.0, 0.788, 0.235),
    vec3(1.0, 0.96, 0.86),
    vec3(1.0, 0.730, 0.180),
    vec3(0.78, 0.79, 0.88),
    vec3(0.22, 0.26, 0.36)
  );

  vec3 seaDeep = sCol(
    vec3(0.040, 0.060, 0.130),
    vec3(0.050, 0.100, 0.280),
    vec3(0.043, 0.071, 0.149),
    vec3(0.004, 0.008, 0.025),
    vec3(0.020, 0.035, 0.075)
  );

  vec3 seaShlo = sCol(
    vec3(0.106, 0.165, 0.369),
    vec3(0.140, 0.240, 0.520),
    vec3(0.090, 0.130, 0.300),
    vec3(0.043, 0.071, 0.149),
    vec3(0.060, 0.090, 0.160)
  );

  vec3 fogCol = sCol(
    vec3(0.560, 0.520, 0.480),
    vec3(0.550, 0.620, 0.850),
    vec3(0.450, 0.400, 0.260),
    vec3(0.020, 0.033, 0.070),
    vec3(0.100, 0.130, 0.190)
  );
  ...
  float fogDen = sF(0.012, 0.010, 0.014, 0.034, 0.046);
  ...
  if (rd.y < 0.0) {
    ...
    if (moonAmt > 0.04) {
      ...
      reflSky += vec3(0.78, 0.80, 0.90) * pow(rMoon, 120.0) * 0.78 * moonAmt;
    }
    ...
    if (moonAmt > 0.04) {
      ...
      col += vec3(0.78, 0.80, 0.90) * mSpec * 0.09 * moonAmt;
    }
